...
<div class="dropdown-content">
          <a href="http://192.168.33.126/About US.html">About US</a>
...
<div class="dropdown-content">
          <a href="http://192.168.33.126/USER%20VOTE.php">Hostel Circle to Main Gate</a>
          <a href="http://192.168.33.126/USER%20VOTE1.php">MainGate to Hostel Circle</a>
          <a href="http://192.168.33.126/USER%20VOTE2.php">Hostel Circle to New Hostels</a>
          <a href="http://192.168.33.126/USER%20VOTE3.php">New Hostels to Hostel Circle</a>
        </div>
</li>
      
    </ul>
   
    <h1 style="font-size:200%;margin-left:40%;margin-top:2%;">New Hostels to Hostel Circle</h1>
...
$sql = "SELECT yes_count, no_count FROM vote_counts3";
...
?>
    ...
    <?php
    $sourceHostelCircle = "SELECT BUS_4, BUS_5 FROM Route_2 WHERE ID = 3;";
    ...
    if ($routeResult && $routeResult->num_rows > 0) {
        ...
        while ($row = $routeResult->fetch_row()) {
            ...
            if ($minutesDifference1 >=5 && $minutesDifference1 <=8 && $hoursDifference1 == 0 && $startDateTime >= $endDateTime1) {
                ...
                $yes_count = 0;
                $no_count = 0;
                $sql = "UPDATE vote_counts3 SET yes_count = 0, no_count = 0";
                $result = mysqli_query($conn, $sql);
  ...
  $no_prcntge=$no_count*100/($yes_count+$no_count);
   ...
   echo '<p style="font-size: 150%; color: purple; margin-left: 45%; margin-top: 3%;">YES PERCENTAGE: ' . $yes_prcntge . '<br></p>';
    ...
    if($vote_given==1){
     echo '<script>
        setTimeout(function() {
            window.location.href = "http://192.168.33.126/BUS%20FRONT_PAGE.html";
        }, 3000);
    </script>';
}
?>
